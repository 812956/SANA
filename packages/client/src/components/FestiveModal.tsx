...
export const FestiveModal = ({ isOpen, onClose, title, children, footer }: FestiveModalProps) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            {/* Modal Content */}
            <div className="glass-panel w-full max-w-lg rounded-2xl relative shadow-glass-lg border border-white/20 animate-scale-in !bg-[#0b1120] !bg-opacity-95">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-white/10">
                    ...
                    <button 
                        onClick={onClose}
                        className="text-white/70 hover:text-white hover:bg-white/10 rounded-full p-2 transition-smooth hover:scale-110"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                {/* Body */}
                <div className="p-6 text-slate-100 font-sans leading-relaxed text-lg">
                    {children}
                </div>
